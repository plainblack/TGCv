import { baseSchemaId, baseSchemaCreatedAt, baseSchemaUpdatedAt, dbVarChar, zodString, dbEnum, dbUuid, dbBoolean, dbRelation, dbMediumText } from '../helpers.mjs';

export const userSchema = {
    kind: 'User',
    tableName: 'users',
    owner: ['$id', 'admin'],
    props: [
        { ...baseSchemaId },
        { ...baseSchemaCreatedAt },
        { ...baseSchemaUpdatedAt },
        {
            type: "string",
            name: "username",
            required: true,
            filterQuery: true,
            unique: true,
            length: 60,
            default: '',
            db: (prop) => dbVarChar(prop),
            zod: (prop) => zodString(prop),
            view: [],
            edit: ['owner'],
        },
        {
            type: "string",
            name: "email",
            required: true,
            filterQuery: true,
            unique: true,
            length: 256,
            default: '',
            db: (prop) => dbVarChar(prop),
            zod: (prop) => zodString(prop).email(),
            view: [],
            edit: ['owner'],
        },
        {
            type: "string",
            name: "realName",
            required: true,
            filterQuery: true,
            length: 60,
            default: '',
            db: (prop) => dbVarChar(prop),
            zod: (prop) => zodString(prop),
            view: [],
            edit: ['owner'],
        },
        {
            type: "string",
            name: "password",
            length: 256,
            required: false,
            default: 'no-password-specified',
            db: (prop) => dbVarChar(prop),
            view: [],
            edit: [],
        },
        {
            type: "enum",
            name: "passwordType",
            required: false,
            default: 'bcrypt',
            db: (prop) => dbEnum(prop),
            enums: ['bcrypt'],
            enumLabels: ['Bcrypt'],
            view: [],
            edit: [],
        },
        {
            type: "enum",
            name: 'useAsDisplayName',
            required: true,
            default: 'username',
            db: (prop) => dbEnum(prop),
            enums: ['username', 'email', 'realName'],
            enumLabels: ['Username', 'Email Address', 'Real Name'],
            view: [],
            edit: ['owner'],
        },
        {
            type: "boolean",
            name: 'verifiedEmail',
            required: true,
            default: false,
            db: (prop) => dbBoolean(prop),
            enums: [false, true],
            enumLabels: ['Not Yet Verified', 'Verified'],
            view: ['owner'],
            edit: ['admin'],
        },
        {
            type: "boolean",
            name: 'admin',
            required: true,
            filterQualifier: true,
            default: false,
            db: (prop) => dbBoolean(prop),
            enums: [false, true],
            enumLabels: ['Not Admin', 'Admin'],
            view: ['owner'],
            edit: ['admin'],
        },
        {
            type: "boolean",
            name: "maintenanceManager",
            required: true,
            filterQualifier: true,
            default: false,
            db: (prop) => dbBoolean(prop),
            enums: [false, true],
            enumLabels: ["Not Maintenance Manager", "Maintenance Manager"],
            view: ["owner"],
            edit: ["admin"],
        },
        {
            type: "boolean",
            name: "productionManager",
            required: true,
            filterQualifier: true,
            default: false,
            db: (prop) => dbBoolean(prop),
            enums: [false, true],
            enumLabels: ["Not Production Manager", "Production Manager"],
            view: ["owner"],
            edit: ["admin"],
        },
        {
            type: "boolean",
            name: 'developer',
            required: true,
            filterQualifier: true,
            default: false,
            db: (prop) => dbBoolean(prop),
            enums: [false, true],
            enumLabels: ['Not a Software Developer', 'Software Developer'],
            view: [],
            edit: ['owner'],
        },
        {
            type: "enum",
            name: 'avatarType',
            required: true,
            default: 'robot',
            db: (prop) => dbEnum(prop),
            enums: ['robot', 'uploaded'],
            enumLabels: ['Robot', 'Uploaded'],
            view: [],
            edit: ['owner'],
        },
        {
            type: "string",
            name: "bio",
            required: false,
            default: '',
            length: 16777215,
            db: (prop) => dbMediumText(prop),
            zod: (prop) => zodString(prop),
            view: ['public'],
            edit: ['owner'],
        },
        {
            type: "id",
            name: 'avatarId',
            required: false,
            db: (prop) => dbRelation(prop),
            relation: {
                acceptedFileExtensions: ['png', 'jpeg', 'jpg', 'gif'],
                type: 'parent',
                name: 'avatar',
                kind: 'S3File',
            },
            default: undefined,
            view: ['public'],
            edit: ['owner'],
        },
        {
            type: "virtual",
            name: 'userId',
            required: false,
            view: ['public'],
            edit: [],
            default: undefined,
            relation: {
                type: 'child',
                name: 'apikeys',
                kind: 'APIKey',
            },
        },
        {
            type: "virtual",
            name: 'userId',
            required: false,
            view: ['public'],
            edit: [],
            default: undefined,
            relation: {
                type: 'child',
                name: 's3files',
                kind: 'S3File',
            },
        },
        {
            type: "string",
            length: 36,
            name: 'tgcUserId',
            unique: true,
            required: false,
            filterQualifier: true,
            db: (prop) => dbUuid(prop),
            default: '',
            view: ['owner'],
            edit: [],
        },
    ],
};

export const RoleOptions = ["admin", "maintenanceManager", "productionManager", "developer", "verifiedEmail"];