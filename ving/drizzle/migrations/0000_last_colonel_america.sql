...
CREATE TABLE `maintenancefiles` (
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	`s3FileId` varchar(36) NOT NULL,
	`maintenanceTicketId` varchar(36) NOT NULL,
...
CREATE TABLE `maintenancetickets` (
	...
	`submittedBy` varchar(64) NOT NULL DEFAULT '',
	`maintenanceTaskId` varchar(36) NOT NULL,
	`maintenanceItemId` varchar(36) NOT NULL,
...
ALTER TABLE `maintenanceschedules` ADD CONSTRAINT `maintenanceschedules_maintenanceItemSet_2bb27d4e_fk` FOREIGN KEY (`maintenanceItemSetId`) REFERENCES `maintenanceitemsets`(`id`) ON DELETE cascade ON UPDATE cascade;--> statement-breakpoint
ALTER TABLE `maintenanceschedules` ADD CONSTRAINT `maintenanceschedules_maintenanceTask_48169202_fk` FOREIGN KEY (`maintenanceTaskId`) REFERENCES `maintenancetasks`(`id`) ON DELETE cascade ON UPDATE cascade;--> statement-breakpoint
