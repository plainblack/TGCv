<template>
    ...
    <div class="mt-3" v-if="maintenancefile.meta?.isOwner">
        <NuxtLink :to="`/maintenancefile/${maintenancefile.props?.id}/edit`" class="no-underline mr-2 mb-2">
            <Button severity="success" title="Edit" alt="Edit Maintenance File"><i class="pi pi-pencil mr-1"></i> Edit</Button>
        </NuxtLink>
        <Button @click="maintenancefile.delete()" severity="danger" title="Delete" alt="Delete Maintenance File"><i class="pi pi-trash mr-1"></i> Delete</Button>
    </div>
</template>
